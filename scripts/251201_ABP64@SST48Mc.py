...
st.go = True   # False #
st.start = 115   # 1 5 30 70 80 115 | 210 315
...
if st(1, 'Save gpx navigation to DB'):
    for folder in (['_raw']):
        gpx2h5([  # '',
            '--db_path', str(path_db),
            '--path', str(path_cruise / 'navigation' / folder / f'{cruise["year"]}*.gpx'),
...
if st(10, f'Save {device} data to DB. Searching {st_prefix}*.csv files'):
    from hdf5_pandas.csv_specific_proc import loaded_sst
    csv2h5(
        [
            "cfg/csv_CTD_SST.ini",
            "--path",
            str(path_cruise / devices[device]["folder"] / "Exported" / f"{st_prefix}*.csv"),
            "--table",
            f"{device}",
            #'--dt_from_utc_hours', '0', #'2'
            "--header",
            "Date(text),Time(text),Pres,Temp,Cond,Sal,SIGMA,O2,O2ppm,SVel,Vbatt",
        ...
        **{  # device_params_dict
            "in": {
                ...
                "csv_specific_param": {
                    ...
                    "O2_fun": lambda O2ppm, Sal, Temp, Pres, Time: DO(
                        do_polyval_time(O2ppm, Time), Sal, Temp, Pres
                    ),
                    "O2ppm_fun": lambda O2ppm, Time: do_polyval_time(O2ppm, Time),
...
if st(20, 'Extract CTD runs to "logRuns" table, filling it with CTD & nav params'):
    ...
    st.go = () != ctd_calc(['cfg/ctd_calc-find_runs.ini',
        ...
        '--min_samples', '600',  # 50 fs*depth/speed = 200: if fs = 10Hz for depth 20m
        '--min_dp', '10',  # 5
...
if st(30, f'Draw {device} data profiles'):  # False: #
    ...
    cfg_in = {
        ...
        "pattern_path": path_cruise / devices[device]["folder"] / "profiles_vsz" / "000000_000000.vsz",
...
if False:  # st(60, 'Extract navigation data at runs/starts to GPX tracks.') # to indicate where no nav?
    h5_t_gpx([
        'cfg/h5_to_gpx_CTDs.ini',
        '--db_path', str(path_db),
        '--tables_list', f"{','.join(devices)}",
