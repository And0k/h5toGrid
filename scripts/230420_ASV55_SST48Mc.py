...
st.go = True   # False #
st.start = 110   # 1 5 30 70 80 115
st.end = 120   # 60 80 120

path_cruise = Path(r'd:\WorkData\BalticSea\230420_ASV55')
...
if st(1, 'Save gpx navigation to DB'):
    ...
    for folder in (['_raw_gpx']):  # /notebook_mat-Lenovo notebook_MSI, 'OpenCPN_Trimble(aft)', 'OpenCPN_Trimble(foredeck)']):
        gpx2h5(['',
            '--db_path', str(path_db),
            '--path', str(path_cruise / 'navigation' / folder / '2023*.gpx'),
...
if st(2, "Save bathymetry to DB from *.xyz export of EA600 echosounder data"):
    ...
    csv2h5(
        ...
        '--min_dict', f'{min_coord}, DepEcho:19',  # 3 # use at least -32768 to replace it by NaN
        '--max_dict', f'{max_coord}, DepEcho:125',  # 300
        '--min_date', '2022-04-20',  # use some value to filter NaTs
        '--corr_time_mode', 'False',  # 'delete_inversions?',
        # '--b_incremental_update', '0',  # '1' coerce to delete data loaded in same table in previous steps (only if previous same log file detected?)
        '--b_interact', '0'
        ],
...
if st(80, 'Gridding'):  # and False: #
    ...
    grid2d_vsz(['cfg/grid2d_vsz.ini', '--db_path', str(path_db),
                ...
                '--max_depth', '150',  # '250',
                '--filter_depth_wavelet_level_int', '9',  # 4, 5, 5, 4, 6, 4, 4, 5
                '--convexing_ctd_bot_edge_max', '15',  # 40 set < bottom because it is harder to recover than delete
                # '--x_resolution', '0.2',
                # '--y_resolution', '5',
                '--depecho_add_float', '0',
