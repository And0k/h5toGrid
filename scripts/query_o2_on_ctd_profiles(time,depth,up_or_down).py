...
path_db = (path_cruise / path_cruise.name.split("@", 1)[0]).with_suffix(r".part-not_cor_O2_v2.h5")
device = "CTD_SST_48Mc#1253"

path_save = Path(r"D:\WorkData\experiment\251202@SST48Mc_calibr(O2)-in_ABP64")
...
with pd.ExcelFile(path_save / "Кислород_ABP64_all.xlsx") as f_xls:
    xls = pd.read_excel(
        f_xls,
        # sheet_name='Лист1',
        ...
        dtype=dtype,
        na_values=[" ", "nan nan", "дно"],  # allows to load required types
    )
...
for date_fmt in ("%d/%m/%Y", "%Y-%m-%d"):
    ...
    b_date_ok = xls.Date_Time.str.match(date_re, na=False)
...
for i, time_st in enumerate(profiles_starts):
    ...
    l.info(
        f"{i}. {data_prof_dict[time_st].index[0]} for {time_st}: diff = "
        f"{time_diff if time_diff > pd.Timedelta(seconds=0) else f'-{-time_diff}'}"
    )
...
for time_st, df_run in data_prof_dict.items():
    ...
    for b_up, sign, df in [(False, 1, df_run.iloc[:idx_P_max]), (True, -1, df_run.iloc[idx_P_max:])]:
        depth_q = query["Depth_ref"]

        # to find next value along movement (a little better accounts for probe delay)
        df = df.sort_values(by=col_P, ascending=not b_up)

        # xls.Depth has NaNs where it corresponds to the maximum depth. Find this Depth value
        b_depth_q = depth_q.notna().values
        idx = -np.ones(b_depth_q.size, dtype=int)
        idx[b_depth_q] = (sign * df[col_P]).searchsorted(sign * depth_q[b_depth_q])
        if any(~b_depth_q):
            assert sum(~b_depth_q) == 1, "Several NaN Depth values - strange data"
            idx[~b_depth_q] = df[col_P].argmax()

            # Update ref data with found Depth value
            p_max = df.iat[idx[~b_depth_q].item(), df.columns.get_indexer([col_P]).item()]
            xls.loc[depth_q.index[~b_depth_q], "Depth_ref"] = p_max
            l.info(f"NaN depth value updated with found maximum: {p_max}")

        df_sel = df.iloc[np.clip(idx, 0, df.shape[0]-1), :]
        l.info(
            f"{df_sel[col_P].tolist()} for {query['Depth_ref'].tolist()} depths on run {'up' if b_up else 'down'}"
        )
        (dfs_up if b_up else dfs_down).append(df_sel)
...
with pd.HDFStore(db_path) if b_save else nullcontext() as db:
    for sfx, df in [("up", df_up), ("down", df_down)]:
        df = pd.concat(
            (
                df[[col_P, col_O, col_S, col_T]].reset_index().rename(columns={"index": "Time"}),
                xls[["Date_Time", "Depth_ref", f"{col_O}_ref", "St"]],
            ),
            axis=1,
...
# %% Not tested (instead data was fitted in Veusz)
profiles_starts = xls.Date_Time[xls.Date_Time.diff() != pd.Timedelta(0)]
