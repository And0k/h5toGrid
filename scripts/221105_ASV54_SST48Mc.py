...
path_cruise = Path(r'd:\WorkData\BalticSea\221105_ASV54')
...
if st(1, 'Save gpx navigation to DB'):
    ...
    for folder in (['_copy(selected_parts_for_loading)']):  #, 'OpenCPN_Trimble(aft)', 'OpenCPN_Trimble(foredeck)']):
        gpx2h5(['',
            '--db_path', str(path_db),
...
if st(10, f'Save {device} data to DB'):
    ...
    csv2h5([
        'cfg/csv_CTD_SST.ini',
        '--path', str(path_cruise / device / '_raw' / '22*[0-9].TOB'),
        '--table', f'{device}',
...
if st(80, 'Gridding'):  # and False: #
    ...
    grid2d_vsz(['cfg/grid2d_vsz.ini', '--db_path', str(path_db),
                ...
                '--begin_from_section_int', '5', #'1',  # values <= 1 means no skip
...
if st(210, f'Save {device} data to DB'):
    ...
    from to_pandas_hdf5.csv_specific_proc import proc_loaded_sst_mws
    ...
    csv2h5([
        ...
        **{'in': {
            ...
            #                        }
            'fun_proc_loaded': proc_loaded_sst_mws,
            'csv_specific_param': {
                'Temp_fun': lambda x: np.polyval([
