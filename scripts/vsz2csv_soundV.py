...
cfg['output_files']['header'] = 'Pres,SoundV'
cfg['output_files']['header'] = 'Pres,SoundV'

cfg['output_files']['precision'] = [3, 2]
cfg['output_files']['name_add'] = '_SoundV.txt'
...
header = [col_name.strip() for col_name in cfg['output_files']['header'].split(',')]
# header_b= [h.encode('ascii') for h in header]
with open(cfg['in']['path'][:-4] + cfg['output_files']['name_add'],
          'w', newline='') as f:  # 'wb',
    writer = csv.writer(f, delimiter='\t')  # ,delimiter=' '
    writer.writerow(header)
    for tup in zip(*[ctd[col_name].round(n_digits) for col_name, n_digits \
                     in zip(header, cfg['output_files']['precision'])]):
        writer.writerow(tup)
print('ok')
