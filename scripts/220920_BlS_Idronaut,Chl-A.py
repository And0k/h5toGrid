...
# ---------------------------------------------------------------------------------------------
raw_subdirs = ['220920', '220921', '220922', '220923']
min_coord = 'Lat:44.28,   Lon:37.7'
max_coord = 'Lat:44.5964, Lon:38.4'  # have slightly bigger spikes
...
if st(5, 'Save NMEA navigation to DB'):
    for folder in raw_subdirs:
        path_raw_pattern = str(path_cruise / r'_raw' / folder / r'ADCP_600kHz\*000n.0*')
        cfg_d.main_call([
            'input.time_interval=[2022-09-20T00:00, 2022-09-24T00:00]',
...
if st(10, f'Save {device} data to DB'):
    for folder, dt_add in zip(raw_subdirs, [
            # Chl-A time                              Idronaut time
            np.datetime64("2022-09-20T15:59:33.66") - np.datetime64('2022-09-20T15:54:33.80'),  # 20e
            np.datetime64("2022-09-21T16:51:29.33") - np.datetime64('2022-09-20T23:33:07.50'),  # 21e
            np.int32(0),  # np.datetime64("2022-09-22T18:07:00.00") - np.datetime64('2022-09-22T18:06:57.00'),  # 22e
            np.int32(0),  # np.datetime64("2022-09-23T17:32:21.66") - np.datetime64('2022-09-23T17:32:18.70')   # 23e  #
            ]):
        csv2h5([
            'cfg/csv_CTD_Idronaut.ini',
            '--path', str(path_cruise / fr'_raw' / folder / device / 'CAST*[0-9].txt'),
            '--table', f'{device}',
            '--dt_from_utc_hours', '3',  # will be substructed
            '--dt_from_utc_seconds', f"{-dt_add.astype('m8[ms]').astype(int) / 1000:.3f}",  # '-550',  # '850'
            '--header',
...
add_col = 'Chl-a'  # ChlA
if st(11, f'Correct {add_col} files: recover time column'):
    import zipfile
    ...
    for folder in raw_subdirs:
        ...
        for f in zp.iterdir():
            print(f.name, '>>>')
            time_end = datetime(*zf.getinfo(f.at).date_time)
            with f.open('r'):
                dat = f.read_text().split()
            # time column generation:
                dat_time = pd.date_range(end=time_end + timedelta(hours=1, seconds=-2), periods=len(dat), freq='333333333N')

            with (path_cruise / fr'_raw' / folder / add_col / f.name).open('wt') as fw:
                for t, d in zip(dat_time, dat):
                    try:
                        d_int = int(re.sub('\D', '', d))
                    except ValueError as e:
                        print('- bad value:', d, e)
                    if d_int < 65536:  # and len(d.strip('0'))
                        fw.write(f'"{t:%d.%m.%Y %H:%M:%S}",{d_int:05}\n')
                    else:
                        print('- bad value:', d, '- too big')
                        continue

            time_start_for_check = datetime(*(int(''.join(c)) for c in grouper(f.name, 2)))
            time_start_for_check = time_start_for_check.replace(year=time_start_for_check.year + 2000)
            times_compare = [dat_time[0], time_start_for_check];
            times_compare.sort(reverse=True)
            dt_check = sub(*times_compare)
            print('Date from file name difference:', dt_check, '(for check only)')


if st(12, f'Save {add_col} data to DB'):
    # Del bad lines before: ^("[0-3]\d\.[01]\d.2022 [0-2]\d:[0-5]\d:[0-5]\d",\d{5})|(.*) with $1 in TextCrawler or whatever
    def reformat_date(txt_ddmmyyyy_hhmmss):
        ddmmyyyy, hhmmss = txt_ddmmyyyy_hhmmss.split(' ', 1)
        return np.datetime64(f"{'-'.join(ddmmyyyy.split('.',2)[::-1])}T{hhmmss}")
    ...
    for folder in raw_subdirs:
        csv2h5([
            ...
            '--header', f'Time(time),{add_col}',  #,SigmaT(float)
            '--comment', 'None',
            # Time,Chlorophyll-a
            # "20.09.2022 10:38:23",14284
            '--delimiter_chars', ',',
            '--b_interact', '0',
            '--skiprows_integer', '0',
            '--min_dict', 'Chl-a:-31',  # replace strange values
            '--max_dict', 'Chl-a:761',
            ], **{ # + common_ctd_params_list,
                'in': {
                    'csv_specific_param': {
                        'Chl-a_fun': lambda c: 0.008719308035714286 * c  # 40/65536*500/35: (mV/code)*((mg/L)/mV)
                        # 761.3261 - 1.0392991E3 * (c * 2.5 / 65536),  # codes to
                        },
                    'converters': {'Time': reformat_date}
                    }
                }
            )

if st(15, f'Add column(s) data from {add_col} table to {device} table DB interpolating by time'):
    import pandas as pd
    from to_pandas_hdf5.h5toh5 import h5move_tables, ReplaceTableKeepingChilds  #, h5index_sort, h5init
    ...
    with pd.HDFStore(path_db, mode='r') as store:
        ...
        df = store[tbl]
        df_add = store[add_col]
        # this is rough (Nearest neighbor) method useful only to get NaN values where they must be according to
        # tolerance - todo: find more specific to this task method
        df = pd.merge_asof(df, df_add, left_index=True, right_index=True, tolerance=pd.Timedelta(1, 's'),
                           direction='nearest')
        for col in df_add.columns:
            df.loc[b_ok, col] = np.interp(df.index[(b_ok := df[col].notna())], df_add.index, df_add[col])

            # write back
    db_temp = path_db.with_name('_not_sorted.h5')
    args_to_hdf_no_index = dict(append=True, data_columns=True, format='table', dropna=True, index=False)
    with pd.HDFStore(db_temp) as store_tmp:
        try:
            del store_tmp[tbl]
        except KeyError:
            pass
...
if st(30, f'Draw {device} data profiles'):  # False: #
    ...
    veuszPropagate.main(['cfg/veuszPropagate.ini',
                         '--path', str(cfg_in['pattern_path'].with_name(f'2' + cfg_in['pattern_path'].name.replace('0', '?')[1:])),  #_*s path_db),
