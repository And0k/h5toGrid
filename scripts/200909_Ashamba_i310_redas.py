...
if st(2, 'Save NMEA navigation to DB'):
    ...
    sys.argv = [__file__,
        ...
        'out.b_incremental_update=0',  # '1' not implemented but would coerce to delete data loaded in same table in previous steps # todo implement
        # '--table_prefix', r'',
        # '--min_date', '2019-07-17T14:00:00',
...
del device_params_dict['in']['fun_proc_loaded']
...
if st(12, f'Save {device} data to DB recorded in terminal mode'):
    if False: # done
        from hdf5_pandas.csv_specific_proc import correct_idronaut_terminal_txt
        from utils.init import open_csv_or_archive_of_them
...
def h5.merge_two_runs(df_log, irow_to, irow_from=None):
    """
