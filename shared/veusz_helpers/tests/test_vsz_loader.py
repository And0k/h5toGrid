...
def test_get_info_from_filename_various_formats(basename, expected_time_start, expected_time_end, expected_devices, comment):
    ...
    time_range, out_info = metadata.get_info_from_filename(basename)
...
def test_add_months():
    ...
    result = metadata.add_months(dt, 3)
...
def test_add_months_with_overflow():
    ...
    result = metadata.add_months(dt, 1)
...
def test_search_time_range_indexes(index, time_range, raw_time_shift_s, expected_result, comment):
    ...
    result = vsz_add_data.search_time_range_indexes(index, time_range, raw_time_shift_s)
...
def test_add_months():
    """Test adding months to a datetime64 object."""
    dt = np.datetime64('2023-01-15T12:00:00')
    result = add_months.add_months(dt, 3)
...
def test_add_months_with_overflow():
    """Test adding months with month overflow."""
    dt = np.datetime64('2023-01-31T12:00:00')
    result = add_months.add_months(dt, 1)
...
def test_zone_to_seconds_offset():
    ...
    # Test UTC+0
    assert vsz_loader.zone_to_seconds_offset("UTC") == 0
    # Test UTC+2 (2 hours in seconds)
    assert vsz_loader.zone_to_seconds_offset("UTC+2") == 7200
    # Test UTC-5 (negative 5 hours in seconds)
    assert vsz_loader.zone_to_seconds_offset("UTC-5") == -18000


def get_path_in_parents_found():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = metadata.get_path_in_parents(child_dir, "target_file.txt")
        assert result == target_file


def get_path_in_parents_not_found():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with pytest.raises(FileNotFoundError):
            metadata.get_path_in_parents(nonexistent_file.parent, "nonexistent.txt")
...
def test_load_info_json():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = load_file_meta.load_info_json(probes, json_file)
