...
def coords(
    ...
    if query_range_lims is None:
        if q_time is None:
            ...
            # Search interval from q_time edges: should be min and max of `q_time` if `query_range_pattern`
...
def keep_recorded_file(
    cur: Mapping[str, Any], existing: pd.DataFrame, keep_newer_records: bool = True, time_range=None
):
    """
