...
from utils.time import timezone_view
from utils.filters import rep2mean, inearestsorted
from hdf5_pandas.csv2h5 import set_filterGlobal_minmax
...
def my_argparser():
    ...
    p = my_argparser_common_part(
        {
            "description": "csv2h5 version {}".format(version)
            + """
----------------------------
...
----------------------------"""
    ...
    s.add("--path_coef", help="path to file with coefficients. Used for processing of Neil Brown CTD data")
    ...
    s.add("--data_columns_list", help="list of columns names used in output csv file(s)")
    s.add("--b_insert_separator", default="True", help="insert NaNs row in table after each file data end")
...
def extractRuns(P: Sequence, cfg_extract_runs: Mapping[str, Any]) -> Tuple[List[int], List[int]]:
    ...
    """
    min_samples = (
        np.array(cfg_extract_runs["min_samples"])
        if ("min_samples" in cfg_extract_runs and cfg_extract_runs["min_samples"])
        else 0
    )
    min_dp = (
        np.array(cfg_extract_runs["min_dp"])
...
def CTDrunsExtract(P: np.ndarray, dnT: np.ndarray, cfg_extract_runs: Dict[str, Any]) -> np.ndarray:
    """
    ...
    if ("do" not in cfg_extract_runs) or cfg_extract_runs["b_do"]:  # not do only if b_do is set to False
        ...
        for ist, ien in zip(time_holes[:-1], time_holes[1:]):
            ...
            if (P[islice].max() - P[islice].min()) < cfg_extract_runs["min_dp"]:
                ...
                if "path_images" in cfg_extract_runs:
                    cfg_extract_runs["path_image"] = (
                        os_path.join(
                            cfg_extract_runs["path_images"],
                            "extract_runs{:%y%m%d_%H%M%S}".format(
                                np.datetime64(dnT[ist], "s").astype(datetime)
                            ),
                        )
                        + ".png"
                    )
        ...
        if len(imin):
            ...
            bok = np.logical_and(
                ...
                np.diff(P[iminmax], 1, 0) >= cfg_extract_runs["min_dp"],
            ).flatten()
...
def load_coef(cfg):
    ...
    cfg["for"]["k_names"], cfg["for"]["kk"] = np.loadtxt(
        cfg["in"]["path_coef"], dtype=[("name", "S10"), ("k", "4<f4")], skiprows=1, unpack=True
    )  # k0	k1*x      	k2*x^2     	k3*x^3	CKO
...
def get_last_finite_index(arr_slice, i_default=0):
    finite_indices = np.flatnonzero(np.isfinite(arr_slice))
    return finite_indices[-1] if len(finite_indices) > 0 else i_default
...
def bad_bot_filter(
    ...
    imin: np.ndarray,
    imax: np.ndarray,
    min_ddens_per_dv=-0.2,
    pres_range=1,
    speed_smooth_sigma=3,
    cfg=None,
):
    """
    Deletes bad values near bottom of profile
    :param df_raw:
    :param imin:
    :param imax:
    :param min_ddens_per_dv:
    :param pres_range:
    :param speed_smooth_sigma:
    :param cfg:
    :return:
    """
    import sys
    ...
    def bad_bot_by_diff(
        ...
        :param speed_down: speed = ∆p/∆time - if provided then time and std_smooth_sigma will not be used
        :param min_ddens_per_dv:
    ...
    def imax_dens_near_bot(
        pres: np.ndarray, dens: np.ndarray, imin: np.ndarray, imax: np.ndarray, pres_range=1
    ):
        # Max dens criteria
    ...
    if "sigma0" in df_raw.columns:
        ...
        t90 = (
            df_raw["Temp90"]
            if "Temp90" in df_raw.columns
            else (
                df_raw["Temp"]
                if cfg["in"].get("b_temp_on_its90")
                else gsw.conversions.t90_from_t68(df_raw["Temp"])
            )
        )
    ...
    dens_filt = bad_bot_by_diff(
        ...
        sigma0,
        imin,
        imax,  # speed_down=None,
        min_ddens_per_dv=-0.2,
        pres_range=1,
        time=(lambda x: x - x[0])(df_raw.index.to_numpy(np.int64)) / 1e9,
        speed_smooth_sigma=3,
    )  # df_raw.index.to_numpy(np.int64) equal to df_raw.index.to_numpy('M8[ns]').astype(np.int64)

    imin_ddens = imin + [get_last_finite_index(dens_filt[slice(*ise)]) for ise in zip(imin, imax + 1)]
...
def log_runs(
    df_raw: pd.DataFrame, cfg: Mapping[str, Any], log: Optional[MutableMapping[str, Any]] = None
) -> pd.DataFrame:
    """
    Changes ``log``
    ...
    imin, imax = CTDrunsExtract(
        df_raw[cfg["extract_runs"]["cols"][0]].values, df_raw.index.values, cfg["extract_runs"]
    )
    if not len(imin):
        return pd.DataFrame(data=None, columns=df_raw.columns, index=df_raw.index[[]])  # empty dataframe

    imax = bad_bot_filter(
        df_raw, imin, imax, min_ddens_per_dv=-0.2, pres_range=1, speed_smooth_sigma=3, cfg=cfg
    )

    if log is None:
        print("not updating log")
        log = {"fileName": None, "fileChangeTime": None}
    else:
        log_loaded_compatible = {
            k: [v] for k, v in log.items() if (not isinstance(v, Sequence)) or isinstance(v, str)
            and len(imax) == 1
        }
        log.clear()
        log.update(log_loaded_compatible)  # if isinstance(v, float)

    log.update({  # pd.DataFrame(, index=log_update['_st'].index).rename_axis('Date0')
        "rows": imax - imin,
        "rows_filtered": np.append(imin[1:], len(df_raw))
        - imax,  # rows between runs down. old: imin - np.append(0, imax[:-1])
    ...
    log.update(
        get_runs_parameters(
            ...
            add_tables_cols={
                cfg["in"]["table_nav"]: ("Lat", "Lon", "DepEcho", "Speed", "Course"),
                cfg["in"]["tables_log"].format(cfg["in"]["tables"][0]): ["fileName_st"],
            },
            dt_search_tolerances=[
                cfg["out"].get("dt_search_nav_tolerance", timedelta(minutes=2)),
                timedelta(hours=2),
            ],
            **{k: cfg["in"].get(k) for k in ["dt_from_utc", "db", "db_path"]},
        )
    )
    ...
    # log.update(
    #     Date0=timezone_view(df_raw.index[imin], cfg['in']['dt_from_utc']),
    #     DateEnd=timezone_view(df_raw.index[imax], cfg['in']['dt_from_utc']),
    #     fileName=[os_path.basename(cfg['in']['file_stem'])]*len(imin),
    #     fileChangeTime=[cfg['in']['fileChangeTime']]*len(imin),
...
def get_runs_parameters(
    ...
    db=None,
    db_path=None,
    add_tables_cols: Mapping[Optional[str], Sequence[str]] = None,
    dt_search_tolerances=timedelta(minutes=2),
) -> Mapping[str, Any]:
    """
    ...
    for times_lim, suffix, log_time_col, i_search in (
        (times_min, "_st", "Date0", 0),
        (times_max, "_en", "DateEnd", -1),
    ):
        log_update[suffix] = df_raw.asof(times_lim, subset=cols_good_data)  # rows of last good data
        log[log_time_col] = timezone_view(log_update[suffix].index, dt_from_utc)
        ...
        for p, *isnan in log_update[suffix].isna().T.itertuples(name=None):
            if i_search == -1:
                ...
                for time_nan, time_min, time_max in zip(times_lim[isnan], times_min[isnan], times_max[isnan]):
                    ...
                    try:
                        log_update[suffix].at[time_nan, p] = s_search[s_search.notna()].iat[
                            0
                        ]  # same as .at[s_search.first_valid_index()]
                    except IndexError:
    ...
    for (table_add, cols_add_), dt_search_tolerance in zip(
        add_tables_cols.items(),
        _ if isinstance(_:=dt_search_tolerances, Sequence) else [_] * len(add_tables_cols),
    ):
        if not table_add:
            continue
        cols_add = []
        ...
        try:
            with FakeContextIfOpen(lambda f: pd.HDFStore(f, mode="r"), db_path, db) as store:
                df_add, dt = h5.load_points(
                    store,
                    table_add,
                    columns=cols_add,
                    time_points=time_points,
                    dt_check_tolerance=dt_search_tolerance,                # warns if'l found far from query
                    to_edge=max(timedelta(hours=2), dt_search_tolerance),  # amount of data to query DB
                )
                # {:0.0f}s'.format(dt_search_tolerance.total_seconds())

                if not any(df_add.columns.difference(df_raw.columns)):
                    continue  # todo: only if close data exist

                # Replacing all NaNs with nearest data in each column
                isnan = df_add.isna()
                for col in df_add.columns[isnan.any(axis=0)]:
                    # Load column from same table again: now all data from our range
                    # todo: replace findilng only positive vals with NaN filtering
                    df_add_col = store.select(
                        table_add,
                        where="index>='{}' & index<='{}' & {} > 0".format(
                            *(time_points + np.array(dt_search_tolerance, "m8[s]") * [-1, 1]), col
                        ),
                        columns=[col],
                    )
                    try:
                        vals = df_add_col[col].values
                        vals = vals[inearestsorted(df_add_col.index, time_points[isnan[col]])]
                    except IndexError:
                        continue  # not found
                    if vals.any():
                        df_add.loc[isnan[col], col] = vals
        except Exception:
            l.exception("Error -> Can not add nav data. Continue...")
            return log  # todo: check if log items not expanded
        # df_add['nearestNav'] = dt.astype('m8[s]').view(np.int64)
        _ = (
            ("_st", df_add.iloc[: len(log_update["_st"])]),
            ("_en", df_add.iloc[len(log_update["_st"]) : len(df_add)]),
        )
        df_edges_items_list = [df_edge.add_suffix(suffix).items() for suffix, df_edge in _]
            ...
            for name, series in st_en:
                ...
                if name in log:
                    ...
                    if b_need.any():
                        ...
                        #     df = cfg['in']['db'].select(cfg['in']['table_add'], where=query_range_pattern.format(st_en.index), columns=name)

                        # df_add = h5select(  # for current parameter's name
                        #     cfg['in']['db'], cfg['in']['table_add'],
                        #     columns=name,
                        #     query_range_lims=st_en
                        #     time_points=log_update['_st'].index.append(log_update['_en'].index),
                        #     dt_check_tolerance=dt_search_tolerance
                        #     )
                    continue
                # else:
...
def coord_data_col_ensure(df: pd.DataFrame, log_row, cfg=None):
    for coord in ["Lat", "Lon"]:
        if coord in df.columns:
            ...
            params_to_calc = set(cfg["out"]["data_columns"]).difference(df.columns)
            if "Lat" in params_to_calc:
                add_nav_params(df, cfg)
                break
        # Not found coord in existed data => Try copy from log_row's start coordinate
        for col in [coord, f"{coord}_st"]:
            try:
                df[coord] = getattr(log_row, col)
                break
            except AttributeError:
                continue
        else:
            l.warning("No nav %s in log table", coord)


def add_ctd_params(df_in: MutableMapping[str, Sequence], cfg: Mapping[str, Any], lon=16.7, lat=55.2):
    """
    Calculate parameters specified in cfg['out']['data_columns'], that could be:
    - 'sigma0', 'depth', 'soundV', 'SA' from existed columns
    ...
    if "Temp90" not in ctd.columns:  # if isinstance(ctd, pd.DataFrame)
        if cfg["in"].get("b_temp_on_its90", True):
            ctd["Temp90"] = ctd["Temp"]
        else:
            ctd["Temp90"] = gsw.conversions.t90_from_t68(df_in["Temp"])
    if "Sal" not in ctd.columns:
        ctd["Sal"] = gsw.SP_from_C(ctd["Cond"], ctd["Temp90"], ctd["Pres"])
    try:
        ctd["SA"] = gsw.SA_from_SP(
            ctd["Sal"], ctd["Pres"], lat=lat, lon=lon
        )  # or Sstar_from_SP() for Baltic where SA=S*
    except (
        NotImplementedError
    ):  # Cannot apply ufunc <ufunc 'sa_from_sp'> to mixed DataFrame and Series inputs
    ...
    if "depth" in params_to_calc:
        ctd["depth"] = np.abs(
            gsw.z_from_p(np.abs(ctd["Pres"].to_numpy()), lat)
        )  # to_numpy() works against NotImplementedError: Cannot apply ufunc <ufunc 'z_from_p'> to mixed DataFrame and Series inputs.
...
def add_nav_params(df: MutableMapping[str, Sequence], cfg: Mapping[str, Any]):
    from utils.filters import inearestsorted_around

    tbl_name = cfg["in"]["table_nav"]
    params_to_calc = set(cfg["out"]["data_columns"]).difference(df.columns)
    cols_add = params_to_calc.intersection(("Lat", "Lon", "DepEcho", "Speed", "Course"))
    ...
    with FakeContextIfOpen(
        ...
        # df_out = pd.concat([df, df_nav], axis=1)
        # Same as above implicetly:
        df[df_nav.columns] = df_nav

        # df_nav, dt = h5.load_points(
        #     store, tbl_name, columns=cols_add, time_points=time_points,
        #     dt_check_tolerance=tol
        # )

        # qstr = "index>='{}' & index<='{}'".format(
        # navp_d['indexs'][0], navp_d['time_poss_max'])
        # nav = cfg['in']['db'].select(tbl_name, qstr, columns=['DepEcho', 'Lat', 'Lon'])

        # df_add_col, dt_col = h5select(
        #         cfg['in']['db'], cfg['in']['table_add'],
        #         columns=[col],
        ...
        # df.join(df_nav, how='left', inplace=True)
        # df_nav_unique = df_nav.groupby(df_nav.index).first()
        # df_out = df.join(df_nav, how='left')  # df.merge on dup index will give cross product


def add_adcp_params(df: MutableMapping[str, Sequence], params_to_calc: Sequence[str]):
    # b_have_u = 'u' in df
    # da_np = da if isinstance(df.get('u' if b_have_u else 'Vabs'), da.Array) else np
    if "u" in params_to_calc or "Ve" in params_to_calc:
        import sys
        ...
        cols_priority = ["Depth", "Vabs", "Vdir", "u", "v"]
        params_to_calc = set(params_to_calc).difference(cols_priority[1:] + ["Ve", "Vn"])
        return df[
            [c for c in cols_priority if c in df.columns] + [c for c in df.columns if c not in cols_priority]
        ], params_to_calc

    return df.sort_values(by=["Depth"]).sort_index(), params_to_calc
...
def main(new_arg=None):
    ...
    try:
        ...
        cfg["in"]["query"] = h5.time_range_query(**cfg["in"])
    ...
    if cfg["out"]["tables"] == ["None"]:
        ...
        cfg["out"]["tables"] = None
        # for _runs cfg will be redefined (this only None case that have sense?):
        # cfg['out']['tables_log'] = None

    h5.out_init(cfg["out"], **cfg["in"])
    # store, df_log_old = h5.temp_open(**cfg['out'])
    ...
    if cfg_fileN.endswith("_runs") or (bool(out_tables_log) and "logRuns" in out_tables_log[0]):
        # Will calculate only after filter  # todo: calculate derived parameters before were they are bad
        # (or replace all of them if any bad?)
        func_before_cycle = lambda x: None
        func_before_filter = lambda df, log_row, cfg: df
        # Update cfg["out"]["log"] by `get_runs_parameters()` internally:
        func_after_filter = lambda df, cfg: log_runs(df, cfg, cfg["out"]["log"])
        # this table will be added:
        cfg["out"]["tables_log"] = [(cfg["out"]["tables"] or cfg["in"]["tables"])[0] + "/logRuns"]
        cfg["out"]["b_log_ready"] = (
            True  # to not update time range in h5.append() from data having only metadata
        )

        # Settings to not affect main data table and switch off not compatible options:
        cfg["out"]["tables"] = []
        cfg["out"]["b_incremental_update"] = (
            False  # todo: If False check it: need delete all previous result of ctd_calc() or set min_time > its last log time. True not implemented?
        )
        ...
        if "brown" in cfg_fileN.lower():
            ...
            def ctd_coord_and_params(df: pd.DataFrame, log_row, cfg):
                coord_data_col_ensure(df, log_row, cfg)
    ...
    with FakeContextIfOpen(
        ...
        for tbl in cfg["in"]["tables"]:
            ...
            df_log = cfg["in"]["db"].select(
                cfg["in"]["tables_log"].format(tbl) or tbl, where=cfg["in"]["query"]
            )
            if True:  # try:
                if "log" in cfg["program"].keys():
                ...
                for ifile, r in enumerate(df_log.itertuples(), start=iSt):  # name=None
                    ...
                    if cfg["in"]["b_incremental_update"]:
                        b_stored_newer, b_stored_dups = h5.del_obsolete(
                            cfg["out"], cfg["out"]["log"], df_log_old
                        )
                        if b_stored_newer:
                            continue
                        if b_stored_dups:
                    ...
                    # Add useful metadata from current row Lat/Lon+ existed metadata values if no Lat/Lon cols
                    # in `df_raw`: Lat/Lon - to cfg['in'], with suffix st/en - to cfg["out"]["log"]
                    if "Lat" not in cols:
                        if "Lat_en" in df_log.columns:
                            cfg["in"]["lat"] = np.nanmean((r.Lat_st, r.Lat_en))
                            cfg["in"]["lon"] = np.nanmean((r.Lon_st, r.Lon_en))
                            for k in ["Lat_st", "Lat_en", "Lon_st", "Lon_en"]:
                                cfg["out"]["log"][k] = getattr(r, k)
                        elif "Lat" in df_log.columns:
                            cfg["in"]["lat"] = r.Lat
                            cfg["in"]["lon"] = r.Lon
                        if "Station" in df_log.columns:  # to copy to runs_log:
                            cfg["out"]["log"]["Station"] = r.Station

                    df = func_before_filter(df_raw, log_row=r, cfg=cfg)

                    if df.size:  # if size == 0 we save only log but not data
                    ...
                    # Append to Store
                    h5.append(cfg["out"], df, cfg["out"]["log"], log_dt_from_utc=cfg["in"]["dt_from_utc"])
                    ...
                    if cfg["out"]["log"].get("Date0") is not None and (
                        ...
                        str_log = (
                            "{fileName}:\t{Date0:%d.%m.%Y %H:%M:%S}-"
                            "{DateEnd:%d.%m %H:%M:%S%z}\t{rows}rows".format_map(cfg["out"]["log"])
        ...
        try:
            if cfg["out"]["tables"] is not None:
                ...
                if cfg["out"]["b_remove_duplicates"]:
                    h5.remove_duplicates(cfg["out"], cfg_table_keys=("tables", "tables_log"))
                # Create full indexes. Must be done because of using ptprepack in h5.move_tables() below
