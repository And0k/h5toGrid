...
def filterGlobal_minmax(a, tim=None, cfg_filter=None, b_ok=True, not_warn_if_no_col=[]) -> pd.Series:
    ...
    for key, v in cfg_filter.items():  # between(left, right, inclusive=True)
        ...
        field = [
            field
            for field in (a.dtype.names if isinstance(a, np.ndarray) else a.columns.to_numpy())
...
def filter_global_minmax(
    a: Union[pd.DataFrame, dd.DataFrame], cfg_filter: Optional[Mapping[str, Any]] = None
...
def filter_local(
    d: Union[pd.DataFrame, dd.DataFrame, Mapping[str, pd.Series], Mapping[str, dd.Series]],
    cfg_filter: Mapping[str, Any],
    ignore_absent: Optional[set] = None,
) -> Union[pd.DataFrame, dd.DataFrame]:
    """
