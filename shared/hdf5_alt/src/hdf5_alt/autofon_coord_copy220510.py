...
from utils.init import Ex_nothing_done, FakeContextIfOpen, set_field_if_no, call_with_valid_kwargs,\
    ExitStatus, GetMutex
...
class ConfigInAutofon:
    ...
    tbl_raw_not_update: bool = False  # for debug: True allows to not load
...
class ConfigProcessAutofon:
    ...
    # List[float] - constant coord. (i.e. [44.56905, 37.97308]), else - empty list to use anchor_coord_time_dict or if last
                ...
                for retry in [False, True]:
                    try:
                        ...
                        if retry:
                            ...
                            continue
                # del data after this in h5.del_obsolete():
...
def main(config: ConfigType) -> None:
    ...
    for b_retry in [False, True]:
        ...
        for i1_tbl, (tbl, tbl_log, t_start, msg_start_origin, _) in h5.dispenser_and_names_gen(
            ...
            if not cfg['in']['tbl_raw_not_update']:
            ...
            if cfg['process']['anchor_coord'][0] == tbl:  # Save tbl data as moving ancnor to can process next device(s)
                cfg['process']['anchor_coord_time_dict'] = df.rename(columns={col: f'{col}_{tbl}' for col in df.columns})
    ...
    # Update output store
    #######################
    if any(h5.unzip_if_need(out['tables_written'])) or cfg['in']['tbl_raw_not_update']:  # cfg_in.get('time_last'):
