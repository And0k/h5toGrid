...
to all elements in the document that don't explicitly use a namespace - so it applies to your <trkpt /> elements as well.
...
def gpx2csv(gpx_file_name):
    ...
    with open(gpx_file_name[:-4] + '.csv', 'w', newline='') as f:  # 'wb',
        ...
        for trkpt in root.iter('{%s}trkpt' % NS):  # '{%s}trkpt' % NS
            ...
            cmt = node_cmt.text.replace(',', ' ') if node_cmt is not None else ""
