...
def chars_array_to_datetimeindex(
    ...
    Bad values (unparseable) are forward-filled from the previous good value.

    Parameters
    ----------
    date
        Numpy array or pandas Series of date strings (may include ``bytes``).
    dtype
        Destination numpy datetime type (e.g. ``np.dtype("datetime64[ns]")``).
    format
        :func:`pandas.to_datetime` format string.
    ...
    try:
        ...
        first = date.iat[0] if is_series else date[0]
        if isinstance(first, bytes):
            decode = date.str.decode if is_series else (lambda **kw: np.char.decode(date, **kw))
            date = decode(encoding="utf-8", errors="replace")
    except Exception:
        pass
    ...
    try:
        ...
        n_bad = t_is_bad.sum()
        if n_bad:
            lf.warning("replacing {:d} bad strings with previous", n_bad)
...
def concat_to_iso8601(a: pd.DataFrame) -> pd.Series:
    """
    Concatenate date/time byte-string parts to ISO8601 strings.

    Parameters
    ----------
    a
        DataFrame with columns ``yyyy``, ``mm``, ``dd``, ``HH``, ``MM``, ``SS``
