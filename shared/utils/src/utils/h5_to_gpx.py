...
def my_argparser():
    ...
    p = my_argparser_common_part({
        "description": "Extract data from Pandas HDF5 store*.h5 files to GPX"
    })  # 'gpx2h5.ini'
    ...
    s = p.add_argument_group(
        "out", "Output files: paths, formats...  - not calculation intensive affecting parameters"
    )
    s.add(
        "--select_from_tablelog_ranges_index",
    ...
    s.add(
        ...
        default="gpx_names_funs[row.itbl](i)",
...
def save_to_gpx(
        nav_df: pd.DataFrame, fileOutPN, gpx_obj_namef=None, waypoint_symbf=None, cfg_proc=None, gpx=None
    ):
    """
            ...
            if cfg_proc.get('simplify_tracks_error_m'):
                try:
                    gpx_segment.points = gpxpy_simplify_polyline(
                        gpx_segment.points, cfg_proc["simplify_tracks_error_m"]
                    )
        ...
        for itbl, tblD in enumerate(cfg['in']['tables']):
            ...
            if cfg['out']['select_from_tablelog_ranges'] is None:
                ...
                for irow, r in enumerate(dfL.itertuples()):  # iterrows()
                    ...
                    try:
                        dfD = store.select(
                            cfg["in"]["table_nav"] if cfg["in"]["table_nav"] else tblD,
                            qstr,
                ...
                time_points = (
                    dfL.index if cfg["out"]["select_from_tablelog_ranges"] == 0 else
