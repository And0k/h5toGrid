...
def check_time_diff(
    t_queried: Union[pd.Series, np.ndarray],
    t_found: Union[pd.Series, np.ndarray],
    dt_warn: Union[pd.Timedelta, np.timedelta64, timedelta],
    msg: str = (
        "Bad nav. data time coverage at {n} points where ∆time[{units}] to nearest data exceeds {dt_warn}:"
    ),
    ...
    if msg:
        ...
        if n:
            if dt_warn > timedelta(minutes=1):
                ...
                dt_div = 1
            msg = "\n".join(
                [msg.format(n=n, dt_warn=dt_warn, units=units)]
                + [
                    "{}. {}:\t{}{:.1f}".format(i, tdat, m, dt / dt_div)
                    for i, tdat, m, dt in zip(
                        np.flatnonzero(bbad)[:max_msg_rows],
                        t_queried[bbad],
                        np.where(dt_arr[bbad].astype(np.int64) < 0, "-", "+"),
