...
def bad_bot_filter(
    ...
    def bad_bot_by_diff(
        ...
        C:\Work\Python\AB_SIO_RAS\Veusz_plugins
    ...
    imin_ddens = imin + [
        np.flatnonzero(np.isfinite(dens_filt[slice(*ise)]))[-1] for ise in zip(imin, imax + 1)
    ]
...
def get_runs_parameters(
    ...
    :param db:
    :param db_path:
    :param add_tables_cols: {tables: cols}: tables names to add starting and ending data from columns
    corresponding columns ``cols``.
    Usually 1st element is 'navigation' table to find data absent in df_raw.
    Note: tries to find only positive vals
    :param dt_search_tolerances: tolerance for each table
    :return: log
    """
    ...
    for (table_add, cols_add_), dt_search_tolerances in zip(
        add_tables_cols.items(),
        dt_search_tolerances
        if isinstance(dt_search_tolerances, Sequence)
        else [dt_search_tolerances] * len(table_add),
    ):
        if not table_add:
            continue
        cols_add = []
        ...
        try:
            with FakeContextIfOpen(lambda f: pd.HDFStore(f, mode="r"), db_path, db) as store:
                df_add, dt = h5.load_points(
                    ...
                    dt_check_tolerance=dt_search_tolerances,
                )
                ...
                # Replacing all NaNs
                isnan = df_add.isna()
                # Search nearst data in each column
                for col in df_add.columns[isnan.any(axis=0)]:
                    ...
                    df_add_col = store.select(
                        ...
                        where="index>='{}' & index<='{}' & {} > 0".format(
                            *(time_points + np.array(dt_search_tolerances, "m8[s]") * [-1, 1]), col
                        ),
                        columns=[col],
                    )
                    try:
            ...
            l.exception("Can not add nav data -> Continue...")
            return log
...
def add_nav_params(df: MutableMapping[str, Sequence], cfg: Mapping[str, Any]):
    from filters import inearestsorted_around
