...
from utils2init import Ex_nothing_done, FakeContextIfOpen, LoggingStyleAdapter, set_field_if_no, call_with_valid_kwargs, ExitStatus, GetMutex
...
def main(config: ConfigType) -> None:
    ...
    if not (tables_written := out['tables_written']):  # what to copy from temp store
        if not out['db_path'].is_file():
            out['tables_written'] = {(tbl, tbl_log)}   # if no output store then recover it by copy all tables from temp
    if cfg['in']['tbl_raw_not_update'] or any(h5.unzip_if_need(tables_written)):  # cfg_in.get('time_last'):
