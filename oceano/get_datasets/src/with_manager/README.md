...
This directory contains Python scripts designed for downloading various types of environmental data from different sources, including Copernicus Marine Service (CMEMS) and ECMWF ERA5 reanalysis. The scripts are structured to be modular and utilize the Hydra configuration framework for flexible parameter management. The logging history and parameters of the download attempt (success or failure)is managed by `DownloadHistoryManager`.

## Configuration Best Practices

The downloading system leverages Hydra for a highly modular and extensible configuration. Instead of modifying script files directly, you define your download tasks in dedicated YAML configuration files.

### Configuration Structure

All YAML configuration files reside in the `cfg/` directory. The core idea is to separate general settings from project-specific ones:
...
2.  Inside your project directory, create a YAML file corresponding to the script you want to use (e.g., `point_wind.yaml` for `download_copernicus_point_wind.py`, or `region.yaml` for `download_copernicus_region.py`).
...
Scripts are executed using the `python` command, with Hydra handling the configuration loading. You specify which project configuration to load using command-line overrides.

### Single Project Execution

To run a script for a specific project, use the `projects=<project_name>/<config_type>` override:

*   **For `download_copernicus_point_wind.py`**:
    ```bash
    python scripts/downloading/with_manager/download_copernicus_point_wind.py projects=kulikovo/point_wind
    ```
    or
    ```bash
    python scripts/downloading/with_manager/download_copernicus_point_wind.py projects=abp56/point_wind
    ```

*   **For `download_copernicus_region.py`**:
    ```bash
    python scripts/downloading/with_manager/download_copernicus_region.py projects=mariculture_1/region
    ```
...
```bash
python scripts/downloading/with_manager/download_copernicus_point_wind.py --multirun projects=kulikovo/point_wind,abp56/point_wind
```

To run all `region` projects:
```bash
python scripts/downloading/with_manager/download_copernicus_region.py --multirun projects=mariculture_1/region,mariculture_2/region,mariculture_3/region,abp56_tchain/region,inflow/region
```
...
### `download_copernicus_point_wind.py`
...
*   **Execution**:
    ```bash
    python scripts/downloading/with_manager/download_copernicus_point_wind.py projects=<project_name>/point_wind
    ```
...
*   **Execution**:
    ```bash
    python scripts/downloading/with_manager/download_copernicus_region.py projects=<project_name>/region
    ```
...
*   **Execution**:
    ```bash
    python scripts/downloading/with_manager/download_copernicus_ftp.py
    ```
    Configuration parameters are loaded from `scripts/downloading/with_manager/cfg/copernicus.yaml` under the `copernicus.ftp` section.
...
```bash
pytest test/test_download_copernicus_point_wind.py
...
```bash
pytest test/test_download_copernicus_point_wind.py::test_point_wind_kulikovo
```
