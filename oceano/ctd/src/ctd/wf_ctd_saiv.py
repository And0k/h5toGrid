...
def proc(common_ctd_params_list, o2_fun=None, o2ppm_fun=None, st_base=200):
    ...
    if st(st_base, f"Save {device} data to DB. Searching {st_prefix}*.csv files"):
        ...
        csv2h5([
            ...
            # '--dt_from_utc_hours', '2',
            '--header', 'No,Meas,Sal,Cond,Temp,O2,O2ppm,Fluor,Turb,Dens,SVel,Pres,Date(text),Time(text)',
    ...
    if st(st_base + 70, "Gridding"):  # and False: #
        ...
        grid2d_vsz([
            ...
            "--data_columns_list", "Turb, Temp, Sal, SigmaTh, O2, O2ppm, soundV",
    ...
    if st(st_base + 100, "Export csv with some new calculated parameters"):  # False: #
        ctd_calc([  # 'ctd_calc-find_runs.ini',
            ...
            "--data_columns_list", "Pres, Temp, Cond, Sal, O2, O2ppm, Turb, Fluor, SA, sigma0, depth, soundV",  # , pH, Eh, Lat, Lon
    ...
    if st(st_base + 105, "Export csv for Obninsk"):
        ...
        h5tocsv([
            ...
            "".join([
                ...
                ", ".join([
                    p if ":" in p else f"{p}: {p}" for p in "Pres;Temp;Cond;Sal;O2;O2ppm;Turb;Fluor;sigma0;soundV".split(";")
