...
python -m meta_finder.post_processing.path_checker_main meta_TCM.tsv
```
...
# Using Python directly
python -m meta_finder.post_processing.path_checker_main --directory ./cruises
```

#### Specify Custom Output Path

```bash
python -m meta_finder.post_processing.path_checker_main meta_TCM.tsv --output path_mapping.tsv
```

#### Specify Custom Similarity Cutoff

```bash
python -m meta_finder.post_processing.path_checker_main meta_TCM.tsv --cutoff 0.5
```

#### Specify Base Search Directory

```bash
python -m meta_finder.post_processing.path_checker_main meta_TCM.tsv --search-dir ./Cruises
```

#### Verbose Mode

```bash
python -m meta_finder.post_processing.path_checker_main meta_TCM.tsv --verbose
```

#### Quiet Mode

```bash
python -m meta_finder.post_processing.path_checker_main meta_TCM.tsv --quiet
```
...
from meta_finder.post_processing.path_checker import check_paths_from_tsv, MATCH_CUTOFF
...
```python
from pathlib import Path
from meta_finder.post_processing.path_checker import (
    read_tsv_paths,
...
```python
from meta_finder.post_processing.path_checker import check_paths_from_tsv
...
No dedicated tests exist yet for these utilities (`tests/test_path_checker*.py` are
not present). Verify manually via `--help` and a dry run on a copy of your TSVs.
