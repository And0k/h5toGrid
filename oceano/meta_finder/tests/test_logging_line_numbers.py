...
@pytest.mark.parametrize(
    "test_case",
    [
        {"id": "basic_logging", "description": "Test basic logging functionality with correct line numbers"},
        {"id": "error_logging", "description": "Test error logging with traceback and correct line numbers"},
        {"id": "custom_logger", "description": "Test CustomLogger class with stacklevel adjustment"},
    ],
)
def test_logging_functionality(test_case):
    ...
    if test_case["id"] == "basic_logging":
        # Test basic logging
        logger = setup_logging("test.basic", use_custom_logger=True, force=True)
        ...
        # Test error logging
        logger = setup_logging("test.error", use_custom_logger=True, force=True)
...
def test_extract_device_time_ranges_error_handling(test_input, expected):
    ...
    with patch("meta_finder.data_proc_funcs.logger") as mock_logger:
        try:
            ...
            assert mock_logger.error.called or True, "Function should log errors appropriately"
        except Exception:
...
@pytest.mark.parametrize(
    ...
    [
        (
            "list_strip_error",
            "Test that line numbers are reported correctly for 'list' object has no attribute 'strip' error",
        ),
        (
            "line_number_accuracy",
            "Test that line numbers in log messages accurately reflect calling locations",
        ),
        ("traceback_inclusion", "Test that error logs include traceback information for better debugging"),
        (
            "exception_line_number_reporting",
...
def test_error_line_number_reporting(error_scenario, expected_description):
    ...
    if error_scenario == "list_strip_error":
        ...
        logger = setup_logging("test.list_strip_error", use_custom_logger=True, force=True)
        ...
        def test_function():
            logger.info(
                "Test message for line number checking"
            )  # This will be logged with actual line number
        ...
        logger = setup_logging("test.traceback_inclusion", use_custom_logger=True, force=True)
        ...
        # Test that when exceptions are caught and re-logged, the original line number is reported
        logger = setup_logging("test.exception_line_number_reporting", use_custom_logger=True, force=True)
        ...
        assert (
            "function_that_raises_exception" in log_content or "function_that_catches_and_logs" in log_content
