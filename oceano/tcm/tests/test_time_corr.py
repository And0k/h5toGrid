...
from tcm.utils_time_corr import (
    ...
    _trim_overlong_runs,
    make_range_mask,
    rms_quantization_theory,
    save_time_corr_diagnostics,
)
...
def _fractional(freq: float, n_secs: int) -> np.ndarray:
    ...
    return np.array([m * NS for m in range(n_secs) for _ in range(N + (1 if m in hi else 0))], np.int64)


def _burst(
    freq: float = 5.0, n: int = 20, blen: int = 10, gap_s: float = 5.0, aligned: bool = False
) -> np.ndarray:
...
class TestRmsQuantization:
    ...
    @pytest.mark.parametrize(
        "N, exp", [(1, 0.0), (2, 0.354), (4, 0.468), (5, 0.490), (6, 0.505), (10, 0.534)]
    )
...
class TestMakeRangeMask:
    ...
    def test_inside_interval(self) -> None:
        t = self._t(0, 300, 600)
        tr = [pd.Timestamp("2000-01-01 10:00", tz="UTC"), pd.Timestamp("2000-01-01 10:10", tz="UTC")]
        assert make_range_mask(t, tr).all()

    def test_outside_interval(self) -> None:
        t = self._t(0, 300)
        tr = [pd.Timestamp("2000-01-01 11:00", tz="UTC"), pd.Timestamp("2000-01-01 12:00", tz="UTC")]
    ...
    def test_multi_range(self) -> None:
        ...
        tr = [
            pd.Timestamp("2000-01-01 10:00", tz="UTC"),
...
class TestEstimateFreq:
    ...
    def test_regime_a_burst_sub_second(self) -> None:
        ...
        t = np.concatenate(
            [
                np.arange(t0, t0 + 10 * dt5, dt5, np.int64)
...
class TestTrimOverlong:
    ...
    def test_prevents_parking_scan_drift(self) -> None:
        ...
        dt = np.int64(NS // 5)
        rms = (
            np.sqrt(np.mean((t2 - _snap_segment_np(t2, np.int64(t2[0]), dt)).astype(float) ** 2)) / NS
        ).item()
...
class TestCorrectTime:
    ...
    def test_without_b_in_range_hwm_is_corrupted(self) -> None:
        ...
        t = np.array(
            ...
            + [base + 5 * NS + i * NS for i in range(5)],
            np.int64,
        )
...
class TestSaveDiagnostics:
    ...
    @pytest.mark.parametrize("diag_first", [0, 10], ids=["no_overlap", "with_overlap"])
    def test_append_merge_across_chunks(self, tmp_path: Path, diag_first: int) -> None:
        """Calling save_time_corr_diagnostics twice on the same path accumulates."""
        t1, t_out1, stats1, act1 = self._data_with_backward()
        p = save_time_corr_diagnostics(t1, t_out1, act1, stats1, {}, str(tmp_path))
        assert p is not None
        d1 = np.load(p)
        n1, k1 = int(d1["n"].item()), len(d1["index"])

        # Chunk 2: different data, optionally with overlap trimmed
        t2, t_out2, stats2, act2 = self._data_with_backward()
        skip = slice(diag_first, None) if diag_first else slice(None)
        save_time_corr_diagnostics(t2[skip], t_out2[skip], act2[skip], stats2, {}, str(tmp_path))

        d2 = np.load(p)
        n2_actual = t2[skip].size
        assert d2["n"].item() == n1 + n2_actual, f"n should be sum of both chunks: {n1} + {n2_actual}"
        # index of chunk 2 is offset by n1
        k2_chunk2 = len(d2["index"]) - k1
        assert k2_chunk2 > 0, "second chunk should add events"
        np.testing.assert_array_equal(d2["index"][k1:], np.load(p)["index"][k1:])  # self-consistent
        assert (d2["index"][k1:] >= n1).all(), f"chunk 2 indices must be offset by n1={n1}"
        # total action + dt_s length matches total index length
        assert len(d2["action"]) == len(d2["index"]) == len(d2["dt_s"])
