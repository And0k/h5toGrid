...
gravity $G_{x}, G_{y}, G_{z}$ and of the magnetic field $B_{x}, B_{y}, B_{z}$ onto the
...
- $\mathbf{G} = [G_{x}, G_{y}, G_{z}]^{T}$ — gravity acceleration vector, g;
- $\mathbf{U}_{G} = [U_{Gx}, U_{Gy}, U_{Gz}]^{T}$ — accelerometer ADC counts.
...
## Zeroing rotation ($R_{z}$)

The sensor's own axes are not necessarily aligned with the instrument's
vertical. The zeroing rotation $R_{z}$ — a $3\times3$ orthogonal matrix —
...
- **From a single vector** (`input.calib.g0xyz`): a raw accelerometer vector
  ...
  $[0, 0, 1]^{T}$ the same way; it overrides any $R_{z}$ from
  `time_ranges_zeroing`.

$R_{z}$ is applied to **both** the accelerometer ($\mathbf{A}_{g}$) and the
magnetometer ($\mathbf{A}_{h}$) calibration matrices, because the two sensors
...
where $f_{v}$ — a polynomial or other increasing function through the origin,
