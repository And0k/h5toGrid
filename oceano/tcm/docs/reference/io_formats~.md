...
### Auxiliary columns handling

``Battery`` and ``TempP`` (pressure-sensor temperature-compensation channel, legacy header
``TP``/``TemprPress``) are **loaded from raw CSV/TXT but excluded from all processed/binned
...
| Output | Battery/TempP |
|--------|:---:|
| ``*.raw.nc`` | **kept** (raw archive) |
| ``*.proc_noAvg.nc`` / ``*.proc_Avg.nc`` / ``*.proc.nc`` | **not saved** |
| ``*.tsv`` | **not saved** |

Drop is unconditional — there is currently no per-probe opt-in; if ``Temp`` or ``Pressure``
should be excluded, use ``out.text_columns`` for TSV or drop them upstream (e.g. by not
providing ``P_t`` / by not loading the column at all via a custom ``text_type``/``text_line_regex``).

### `text_columns` saving policy
