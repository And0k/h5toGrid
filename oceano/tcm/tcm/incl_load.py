...
from hdf5_pandas.csv_specific_proc import mod_name, rep_in_file, correct_txt
from hdf5_pandas.h5_dask_pandas import h5q_interval2coord
...
from utils.init import my_logging, init_logging, open_csv_or_archive_of_them, st, cfg_from_args, \
    my_argparser_common_part, constant_factory
from utils.magn_decl import mag_dec
...
-----------------------------------------------------------"""}, version)
    ...
    s.add('--raw_pattern', default='*{prefix:}{number:0>3}*.[tT][xX][tT]',
          ...
          'where "prefix" is a --probes_prefix arg. that is in UPPER case and INCL replaced with INKL. To find files in archive'
               'subdirectories can be prepended with "/" but files will be unpacked flatten in dir with archive name')
    s.add('--probes_int_list',
          help='Note: Not affects steps 2, 3, set empty list to load all found in possible range 1:41')
...
def correct_files(
    ...
    probes,
    correct_fun,
    fun_correct_name=lambda file_in: mod_name(file_in, add_prefix="@")[1],
    **kwargs,
):
    """
...
def main(new_arg=None, **kwargs):
            ...
            csv2h5(
                [
                    str(
                        ...
                        / "cfg"
                        / f"csv_{p_type[cfg['in']['probes_prefix']]['format']}.ini"
                    ),
                    "--path",
                    str(file_in),  # str(raw_subdir / str(mod_name(raw_pattern_file, add_prefix='@'))),
                    "--blocksize_int",
                    "50_000_000",  # 50Mbt
                    "--table",
                    tbl,
                    ...
                    "0",
                    "--fs_float",
                    str(p_type[cfg["in"]["probes_prefix"]]["fs"]),  # f'{fs(probe, file_in.stem)}',
                    "--dt_from_utc_seconds",
                ...
                + (["--csv_specific_param_dict", "invert_magnetometer: True"] if probe_is_incl else []),
                **{
                    "filter": {
                        "min_date": cfg["filter"]["min_date"][probe],
                        "max_date": cfg["filter"]["max_date"][
                            probe
    ...
    if st(40, f'Draw in Veusz by loader-drawer.vsz method'):
        ...
        veuszPropagate.main([
            ...
            # '--add_custom_list', f'{device_veusz_prefix}USE_time_search_runs',  # 'i3_USE_time
            # '--add_custom_expressions',
            # """'[["{log_row[Index]:%Y-%m-%dT%H:%M:%S}", "{log_row[DateEnd]:%Y-%m-%dT%H
