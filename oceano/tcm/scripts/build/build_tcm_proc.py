...
output (env-suffixed subfolder, e.g. ``B:\\Temp\\tcm_proc-env=noh5-tcm``).
"""
...
_ap.add_argument(
    "--build-root",
    type=Path,
    help="root for build/dist output; '<root>/tcm_proc-env=<pixi env>/' is added (default: in-repo)",
)
WORKPATH, DISTPATH = build_layout("tcm_proc", _ap.parse_args().build_root)
...
write_meta(
    ...
    product="tcm_proc",
    description=(
        "AB SIO RAS' TCM raw data processing CLI. Exports physical values (e.g. water velocity) to text"
    ),
...
if result.returncode == 0:
    print(f"\nBuild complete: {DISTPATH / 'tcm_proc' / 'tcm_proc.exe'}")
