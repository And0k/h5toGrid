...
from hdf5_pandas.csv_specific_proc import correct_kondrashov_txt, rep_in_file, correct_baranov_txt
...
if st(1):  # Can not find additional not corrected files for same probe if already have any corrected in search path (move them out if need)
    ...
    for probe in probes:
        ...
        for in_file in raw_found or open_csv_or_archive_of_them(
            raw_parent / raw_archive_name, binary_mode=False, pattern=raw_pattern.format(probe)
        ):
            ...
            csv2h5(
                [
                    ...
                    re.sub(
                        "^((?P<i>inkl)|w)_0",
                        lambda m: "incl" if m.group("i") else "w",  # correct name
                        re.sub("^[\d_]*|\*", "", in_file.stem).lower(),
                    ),  # remove date-prefix if in name
                    "--min_date",
                    datetime64_str(
                        min_date[probe]
                    ),  # use i_proc_file instead probe if many files for same probe
                    "--max_date",
                    datetime64_str(max_date[probe]),
                    "--db_path",
                    str(db_path),
                ...
                + (
                    ...
                    if prefix == "incl"
                    else ["--cols_load_list", "yyyy,mm,dd,HH,MM,SS,P,U"]
                )
            )
