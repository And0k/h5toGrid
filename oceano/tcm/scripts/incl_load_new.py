...
from utils.init import LoggingStyleAdapter, constant_factory
...
def main(config: ConfigType) -> None:
    ...
    if st(1, 'Save inclinometer or wavegauge data from ASCII to HDF5'):
        ...
        cfg["in"]["dt_from_utc"] = defaultdict(
            constant_factory(cfg["in"]["dt_from_utc"].pop(0, timedelta(0))), cfg["in"]["dt_from_utc"]
        )
                ...
                csv2h5(
                    [
                        str(
                            ...
                            / "cfg"
                            / f"csv_{'inclin' if probe_is_incl else 'wavegauge'}_{p_type[cfg['in']['probes_prefix']]['format']}.ini"
                        ),
                        "--path",
                        str(file_in),
                        "--blocksize_int",
                    ...
                    **{
                        "filter": {
                            ...
                            "max_date": cfg["filter"]["max_date"].get(
    ...
    from tcm.incl_h5clc import h5_names_gen
    from tcm.h5inclinometer_coef import rot_matrix_x  #rotate_x, rotate_y
