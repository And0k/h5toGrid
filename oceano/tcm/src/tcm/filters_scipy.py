from __future__ import annotations  # turns on delayed annotations in python 3.12 - critical for matplotlib
from typing import Tuple, Optional, TYPE_CHECKING
import numpy as np

try:
    from scipy.ndimage import gaussian_filter1d as gaussian_smooth
except ImportError:
...
#@njit: gaussian_filter1d() not supported, todo: check simpler gaussian_smooth variants
def despike(
    x: np.ndarray,
    ...
    ax=None,
    label="",
    std_smooth_sigma=None,
    x_plot=None,
) -> Tuple[np.ndarray, Optional[Axes]]:
    r"""
    ...
    for i, (offset, block) in enumerate(offsets_blocks):
        ...
        if std_smooth_sigma:
            std = gaussian_smooth(std, std_smooth_sigma)

        y = np.ma.masked_where(np.abs(y - mean) > offset * std, y, copy=False)

        if __debug__:
            n_filtered.append(y.mask.sum().item())
            if ax is not None:
    ...
    if __debug__:
        print(
            f"despike({offsets = }, {blocks = }) deletes",
            "->".join(str(n) for n in n_filtered),
            " points",
        )
        if ax is not None:
