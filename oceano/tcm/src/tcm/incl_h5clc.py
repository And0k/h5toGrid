...
from utils.init import Ex_nothing_done, call_with_valid_kwargs, set_field_if_no, init_logging, cfg_from_args, \
...
def recover_magnetometer_x(Mcnts, Ah, Ch, max_h_minus_1, len_data):
    ...
    if can_recover:
        ...
        l.info(
            "Magnetometer x channel %s: %d bad where y&z is ok. y&z ok in %d/%d",
            "recovering" if need_recover else "checked - ok",
            need_recover,
            can_recover,
            len_data,
        )
        if need_recover:  # have poins where recover is needed and is posible
            ...
            if np.isnan(mean_HsumMinus1) or (
                np.fabs(mean_HsumMinus1) > 0.5 and need_recover / len_data > 0.95
            ):
                l.warning(
                    "mean_Hsum is mostly bad (mean=%g), most of data need to be recovered (%g%%) so no trust "
                    "it at all. Recovering all x-ch.data with setting mean_Hsum = 1",
                    mean_HsumMinus1,
                    100 * need_recover / len_data,
                )
                bad = da.ones_like(
                    HsumMinus1, dtype=np.bool_
            ...
            Mcnts_list[0] = da.from_array(
                rep2mean_da2np(Mcnts_list[0], ~bad), chunks=Mcnts_list[0].chunks, name="Mcnts_list[0]"
            )
        else:
        ...
        for ch, i in [('x', 0), ('y', 1), ('z', 2)]:  # in ([('y', 1), ('z', 2)] if need_recover else
            ...
            if n_bad:
                ...
                if n_good / n_bad > 0.01:
                    l.info(
                        f"channel {ch}: bad points: {n_bad} - recovering using nearest good points ({n_good})"
                    )
                    Mcnts_list[i] = da.from_array(
                        rep2mean_da2np(Mcnts_list[i], ~bad),
                        chunks=Mcnts_list[0].chunks,
...
def incl_calc_velocity(
    ...
    Ah: Optional[np.ndarray] = None,
    Ch: Optional[np.ndarray] = None,
    kVabs: Optional[np.ndarray] = None,
    azimuth_shift_deg: Optional[float] = None,
    **kwargs,
) -> dd.DataFrame:
    """
...
def h5_names_gen(cfg_out, tables, db_path, **kwargs
    ...
    :param db_path: hdf5 file with tables which have coef group nodes.
    :param cfg_out: stores `tables_found` - sets to list of found input tables in the store
    :return: iterator that returns (table name, coefficients). "Vabs0" coef. will be replaced with "kVabs"
    updates cfg_out['tables_found']
    """
...
def h5_append_to(
    ...
    if dfs is not None:
        ...
        try:
            ...
            pass
        h5.append(
            {**cfg_out, **tables_dict}, dfs, {} if log is None else log
        )  # , cfg_out['log'], log_dt_from_utc=cfg_in['dt_from_utc'], 'tables': None, 'tables_log': None
...
def gen_subconfigs(
    ...
    - db_path: Union[str, Path], str/path of real path or multiple paths joined by '|' to be splitted in listg['db_paths'],
    - table:
    - min_date:
    - max_date:
    ...
    for d_source in gen_sources_dict(cfg_many):
        ...
        for itbl, (tbl, coefs) in h5.dispenser_and_names_gen(
            fun_gen, cfg_out, b_close_at_end=False, tables=[cfg_in_copy['table']], **cfg_in_copy
        ):
            l.info('%s. %s: ', n, tbl)  # itbl
            cfg_in_copy['table'] = tbl                           # for gen_data_on_intervals()
            # cfg_in_copy['tables'] = cfg_out["tables_found"]
...
""")
    ...
    for itbl, tbl, coefs, d in gen_subconfigs(cfg['out'], fun_gen=h5_names_gen, **cfg['in']):
        ...
        if not cfg['in']['db_path'].stem.endswith('proc_noAvg'):
            ...
            if cfg['in']['time_range_zeroing']:
                ...
                if time_range_zeroing:
                    ...
                    l.info(
                        "Zeroing data: average %d points in interval %s - %s",
                        len(d_zeroing),
                        d_zeroing.divisions[0],
                        d_zeroing.divisions[-1],
                    )
        ...
        if cfg['in']['db_path'].stem.endswith('proc_noAvg'):  # not binned data was loaded => it have bin binned here
            ...
            d = calc_pressure(d, **{(pb := "bad_p_at_bursts_starts_period"): cfg["filter"][pb]}, **coefs)
        ...
        dd_to_csv(
            ...
            cfg["out"]["text_columns"],
            cfg["out"]["aggregate_period"],
            suffix=tbl,
            b_single_file=not cfg["out"]["split_period"],
        )
    ...
    if dfs_all is not None and len(cfg['out']['tables_written']) > 1:
        call_with_valid_kwargs(
            dd_to_csv,
            (
                lambda x: x.resample(rule=aggregate_period_timedelta).first().fillna(0)
