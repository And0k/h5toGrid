...
def add_tz_if_need(v, tim):
    ...
    :param tim: series/index from which tz will be copied
...
def ini2dict(arg_source: Union[Mapping[str, Any], str, PurePath, None] = None):
    ...
    When *arg_source* is already a ``Mapping`` (e.g. a Hydra ``DictConfig``
    passed from :func:`tcm.cli.main_init`), no file I/O occurs — the
    existing mapping is used directly.  Otherwise a ``.ini`` or ``.yaml``
    file is loaded via :func:`cfgfile2dict`.
    ...
    ----------
    arg_source
        Config source.  ``Mapping`` → used as-is.  ``str`` / ``PurePath``
        → loaded as ``.ini`` or ``.yaml``.  ``None`` → program-name-based
        ``.ini`` file.
...
def name_output_and_log(out_path=None,
    ...
    :param f_rep_filemask: function f(path) modifying its argument
        To replase in 'filemask' string '<File_in>' with base of cfg['in']['path'] use
    lambda fmask fmask.replace(
            '<File_in>', os_path.splitext(os_path.basename(cfg['in']['path'])[0] + '+')
    :return: path, ext, l
...
def open_csv_or_archive_of_them(filename: Union[PurePath, Iterable[Union[Path, str]]], binary_mode=False,
    ...
    if hasattr(filename, '__iter__') and not isinstance(filename, (str, bytes)):
        ...
        if arc_suffix:
            ...
            if ptn_re and not re.match(ptn_re, filename_str):
                return
