...
from utils.filters import b1spike_up
from hdf5_pandas.h5_dask_pandas import i_bursts_starts, h5.append
from hdf5_pandas import h5

# to save through temporary store (with log for each saving data part - here it is burst):
from hdf5_pandas import h5
from utils.init import this_prog_basename, standard_error_info, LoggingStyleAdapter
...
def main(config: ConfigType) -> None:  #
    ...
    logger = logging.getLogger('hdf5_pandas.h5_dask_pandas')
    ...
    def names_gen(cfg_out: Mapping[str, Any], db_path, **kwargs) -> Iterator[None]:
        #cfg_out['log']['fileName'] = pname.name[-cfg_out['logfield_fileName_len']:-4]
        cfg_out['log']['fileChangeTime'] = datetime.fromtimestamp(db_path.stat().st_mtime)
        yield None

    h5.out_init(cfg_in, cfg_out)  # cfg_in for full path if cfg_out['db_path'] only name
    n_rows_after = 0
    #with pd.HDFStore(out_path) as store:  #, mode='w'
    for _, _ in h5.dispenser_and_names_gen(names_gen, cfg_out, **cfg_in):  # handles temporary db for h5.append()
        try:
