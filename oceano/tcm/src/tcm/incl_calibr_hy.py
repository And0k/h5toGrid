...
from tcm.h5inclinometer_coef import h5copy_coef, channel_cols, dict_matrices_for_h5
from tcm.incl_h5clc_hy import incl_calc_velocity_nodask, gen_subconfigs, probes_gen
from hdf5_pandas.h5_dask_pandas import filter_local
from utils.init import this_prog_basename, standard_error_info, my_logging
...
def bin_avg_3d_partial(index: np.ndarray | pd.DatetimeIndex, vec3d: np.ndarray, bins=200):
    ...
    vec3d = np.column_stack([
        vec3d[:, slice(*i_st_en)].mean(axis=1) if b_need else vec3d[:, i_st_en[0]] for
...
def main(config: ConfigType) -> None:
    ...
    for d, _, tbl, probe_number_str in gen_subconfigs(
        ...
        for channel in cfg['in']['channels']:
            ...
            fig_filt.axes[0].plot(
                index_avg, vec3d_avg[2, :], label='distributed 3D avg,\nnear to sphere',
