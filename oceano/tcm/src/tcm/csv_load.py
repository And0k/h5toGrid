...
def _compress_times(times: Iterable[Any], limit: int = 20, fmt="%Y-%m-%d %H:%M:%S") -> str:
    ...
    _NS = 1_000_000_000
    ts_iter = (datetime.fromtimestamp(int(t) // _NS, tz=UTC) for t in times)
    try:
        first = next(ts_iter)
...
def _pattern_to_regex(name: str) -> str:
    ...
    Rules (see docs/project_developer_guide/CLI.md → "Path pattern interpretation"):
...
def init_input_cols(cfg_in: MutableMapping[str, Any] | None = None):
    """
    ...
    if b_was_no_dtype:
        ...
        cfg_in["dtype"] = np.array(cfg_in["dtype"], f"|S{max(len(dtype_text_max), numpy_cur_dtype_len):.0f}")
    ...
    for i, s in enumerate(cfg_in["cols"]):
        if len(s) == 0:
            ...
            b_i_not_in_converters = (i not in cfg_in["converters"].keys()) if cfg_in["converters"] else True
            i_suffix = s.rfind("(text)")
            if i_suffix > 0:  # text
                cfg_in["cols"][i] = s[:i_suffix]
                if (
                    cfg_in["dtype"][i] == np.float64
                ) and b_i_not_in_converters:  # reassign from default float64 to text
    ...
    if "cols_loaded_save_b" in cfg_in:  # list to array
        ...
        if "coldate" in cfg_in:
            cfg_in["cols_loaded_save_b"][cfg_in["dtype"].names.index(cfg_in["cols"][cfg_in["coldate"]])] = (
                False
            )

    # Exclude index from cols_loaded_save_b
    if b_index_exist and cfg_in["col_index_name"]:
        cfg_in["cols_loaded_save_b"][cfg_in["dtype"].names.index(cfg_in["col_index_name"])] = (
            False  # (must index be used separately?)
        )
...
def csv_process(
    ...
    # Pass overlap size so diagnostics skip the duplicated t_prev region
    time_cor, b_ok = utils_time_corr.time_corr(
        date,
        cfg_in,
        process=not first_last_row,
        diag_first=t_prev.shape[0] if b_overlap else 0,
    )  # may be long
    if b_overlap:
        _ = t_prev.shape[0]
        time_cor = time_cor[_:]
...
def extract_csv_edges(
    path: Path, mult: int = 5, skiprows: int | None = 0, chunksize: int | None = None, **read_csv_args
) -> pd.DataFrame:
...
def csv_read_gen(
    files: Sequence[str | Path], first_last_row: bool = False, **cfg_in: Mapping[str, Any]
) -> Iterator[tuple[int, Path, pd.DataFrame | None]]:
    """
    ...
    read_csv_args.update({"skipinitialspace": True, "usecols": cfg_in["dtype"].names, "header": None})
    # Removing "ParserWarning: Both a converter and dtype were specified for column k..."
    if read_csv_args["converters"]:
        read_csv_args["dtype"] = {
            k: v[0]
            for i, (k, v) in enumerate(read_csv_args["dtype"].fields.items())
    ...
    for i1_path, path in enumerate(files, start=1):
        ...
        for i_retry in [False, True]:  # try again on ParserError with tuned loading parameters
            try:
                ...
                if (
                    read_csv_args["on_bad_lines"] not in ("warn", "skip")
        ...
        # Generate final consolidated diagnostics plot from accumulated NPZ.
        # save_time_corr_diagnostics merges chunks via append-merge; the plot
        # is produced once here (t_obs_ns=None → sample-index x-axis, correct
        # for the merged multi-chunk index offsets).
        if npz_path := cfg_in.pop("_diag_npz_path", None):
            try:
                from tcm.plot_time_corr_diagnostics import plot_time_corr_diagnostics

                plot_time_corr_diagnostics(Path(npz_path), t_obs_ns=None, path_save=None)
            except Exception:
                lf.debug("Final diagnostics plot not saved", exc_info=True)
...
def format_parts_select_raw(
    file_path: Path, max_bad_top_rows=100, min_good_cols=12, sep=","
...
def correct_raw_files(
    csv_files: list[Path],
    text_type: str,
    text_line_regex: bytes | None = None,
    ...
    if n_corrected:
        lf.info("Corrected {:d} file{:s}{:s}", n_corrected, "" if n_corrected == 1 else "s", probe_label)
    else:
...
def load_from_csv_gen(
    ...
    cfg_in_probe: Mapping[str, Any] | None = None,
    skip_for_meta: Callable[[Any], bool] | None = None,
    return_=None,
    ...
    if cfg_in.get("fun_proc_loaded") is None:
        ...
        if "coldate" not in cfg_in:  # if Time includes Date then we will just return it
            ...
            cfg_in["fun_proc_loaded"] = lambda a, cfg_in, dummy=None: (
                a["Date"] + np.array(np.int32(1000 * a[cfg_in["col_index_name"]]), dtype="m8[ms]")
            )
