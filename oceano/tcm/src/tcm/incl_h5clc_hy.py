...
def f_linear_k(x0, g, g_coefs):
    ...
    return min(
        rep_if_bad(np.diff(g(x0 - np.float64([0.01, 0]), g_coefs)).item() / 0.01, replacement),
        replacement
    )
...
def fG(
    Axyz: Union[np.ndarray, da.Array],
...
def probes_gen(cfg_out, tables, db_path
    ...
    :param cfg_out: keeps cfg_out['tables_found'] equal to list of found tables in store
    ...
    Updates cfg_out['tables_found'] - sets to list of found tables in store
    """
    cfg_out['tables_found'] = None
    with pd.HDFStore(db_path, mode='r') as store:
        cfg_out['tables_found'] = h5.find_tables(store, tables[0]) if len(tables) == 1 else tables
        for tbl in cfg_out['tables_found']:
            if '.proc_noAvg' in db_path.suffixes[-2:-1]:
