...
- Секция "`out`"
  - `db_name` - имя выходного hdf5 файла 1го этапа работы программы (см [ниже](#Программа-работает-в-два-этапа)), к hdf5 файлам следующего этапа добавляются соответствующие суффиксы. По умолчанию "incl.h5" с приставкой из цифр если `path_cruise` с них начинается (у меня названия директорий данных вначале содержат время их начала ггmmdd)
...
```
Перейти в каталог программы обработки инклинометров (замените на свой - куда вы скачали пакет, и для перехода на другой диск в Windows служит команда "`буква диска`:"):
```cmd
d: && cd d:\Work\_Python3\And0K\h5toGrid\inclinometer
```
Запустить программу указав первым параметром путь к файлу конфигурации:
```cmd
...
```cmd
python -m incl_load tests/data/inclinometer/200813incl_load#b-sent.yml --step_start_int 2
```
