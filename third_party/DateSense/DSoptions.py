...
class DSoptions(object):
    ...
    rule_delim_date = DSDelimiterRule(('%d', '%m', '%y', '%Y'), ('-', '/'),
                                      posscore=2)  # Days, months, years usually delimited by '-' or '/'
    rule_delim_ISO_date = DSDelimiterRule(('%G', '%g', '%V', '%u', '%j'), ('-', 'W'), posscore=2,
                                          negscore=-3)  # ISO 8601 date directives should always be adjacent to '-' (or sometimes 'W')
    rule_delim_time = DSDelimiterRule(('%H', '%I', '%M', '%S'), ':',
                                      posscore=2)  # hours, minutes, seconds usually delimited by ':'
    rule_range_years = DSLikelyRangeRule(('%Y', '%G'), (1000, 3000), posscore=1,
                                         negscore=-1)  # Year is probably between 1000 and 3000
    rule_range_cent = DSLikelyRangeRule('%C', (10, 30), negscore=-1)  # Century is probably between 10 and 30
    rule_range_secs = DSLikelyRangeRule('%S', (0, 59),
                                        negscore=-1)  # Seconds CAN be 60 or 61 due to leap seconds, but that's very rare. Normally it'll be in the range (0, 59).
    ...
    rule_pattern_US_date = DSPatternRule((('%m', '%b', '%B'), '%d', ('%y', '%Y')), 2,
                                         posscore=4)  # American - months then days then years
    rule_pattern_US_Bd = DSPatternRule(('%d', ('%m', '%b', '%B'), ('%y', '%Y')), 2,
                                       posscore=3)  # European - days then months then years
    ...
    rule_pattern_verbose_month = DSPatternRule(('month', ('%m', '%b', '%B')), 4,
                                               posscore=2)  # Word "month" typically precedes the month
    rule_pattern_verbose_year = DSPatternRule(('year', ('%Y', '%y')), 4,
                                              posscore=2)  # Word "year" typically precedes the year
    rule_pattern_verbose_time = DSPatternRule(('time', ('%H', '%I')), 4,
                                              posscore=2)  # Word "time" typically precedes the time
    rule_pattern_ISO_W = DSPatternRule(('W', '%V'), 1, posscore=2)  # ISO 8601 date formats often include W%V
    rule_pattern_ISO_date = DSPatternRule((('%G', '%g'), '-', 'W', '%V', '-', '%u'), 1,
                                          posscore=4)  # ISO 8601 date with week number
    ...
    def cull_with_date_tokens(self, date_tokens):
        ...
        for i in range(0, itrrange):
            for j in range(len(self.allowed[i]) - 1, -1,
                           -1):  # iterate backwards so we can remove elements without hiccuping
    ...
    def get_long_debug_string(self, tokdelimiter=', ', posdelimiter='\n'):
        ...
        for toklist in self.allowed:
            ...
            if len(toklist):
                firsttok = True
                toklist.sort(key=lambda
                    DStoken: -DStoken.score)  # Sort the list by descending score to make it more immediately readable
                for tok in toklist:
                    if firsttok:
                        firsttok = False
                    else:
